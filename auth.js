...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    res.json({ token, user:{ id:user.id, name:user.name, username:user.username, role:user.role, supplier_name:user.supplier_name } });
...
router.get('/users', authMiddleware, requireRole('admin'), async (req, res) => {
  const [rows] = await pool.query('SELECT id,name,username,role,supplier_name,created_at FROM users ORDER BY created_at DESC');
  res.json(rows);
});
...
router.post('/users', authMiddleware, requireRole('admin'), async (req, res) => {
  ...
  try {
    ...
    await pool.query('INSERT INTO users (name,username,password_hash,role,supplier_name) VALUES (?,?,?,?,?)',
